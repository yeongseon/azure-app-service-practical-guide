...
Purpose:
    ...
    or secret tokens appear in tracked lab artifacts. The sanitizer in
    ``labs/_shared/scripts/sanitize-artifacts.py`` is the local defense;
    this scanner is the repo-side merge gate.
...
def main(argv: list[str] | None = None) -> int:
    ...
    if findings:
        print(
            "[scan_lab_pii] PII found. Sanitize with "
            "labs/_shared/scripts/sanitize-artifacts.py before committing.",
            file=sys.stderr,
        )
        return 1
    return 0
