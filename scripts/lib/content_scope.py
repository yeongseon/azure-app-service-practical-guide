...
# these sections (for example ``docs/start-here/``, ``docs/visualization/``,
# ``docs/language-guides/``, ``docs/reference/``, ``docs/contributing/``,
# ``docs/meta/``, ``docs/about.md``, ``docs/index.md``) are out of scope
...
# ``platform/architecture/index.md`` and ``platform/containers/index.md``) are
...
def is_in_scope(rel_path: Union[Path, str]) -> bool:
    ...
    True
    >>> is_in_scope("platform/containers/index.md")  # factual subsection landing
    True
...
def is_tautological_text(text: object) -> bool:
    ...
    True
    >>> is_tautological_text("App Service supports VNet integration")
    False
