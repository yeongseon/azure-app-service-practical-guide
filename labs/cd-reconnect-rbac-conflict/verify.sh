...
[ "$INITIAL_COUNT" -ge 1 ] || { echo "FAIL: no pre-existing AcrPull assignment for $APP_NAME on $ACR_NAME - cannot reproduce"; exit 1; }
...
ASSIGNMENT_ID=$(az role assignment list --assignee "$APP_PRINCIPAL_ID" --scope "$ACR_ID" --query "[0].name" --output tsv | tr -d '\r')
...
sleep 15
FINAL_COUNT=$(az role assignment list --assignee "$APP_PRINCIPAL_ID" --scope "$ACR_ID" --query "length(@)" --output tsv | tr -d '\r')
if [ "$FINAL_COUNT" = "1" ]; then
    echo "PASS: recovery successful - 1 active AcrPull assignment for $APP_NAME on $ACR_NAME"
    exit 0
else
