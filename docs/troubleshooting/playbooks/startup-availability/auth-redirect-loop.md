...
#### Portal view: Runtime log stream for live redirect-loop traffic
...
The `Log stream` blade with the `Runtime` radio selected is the live tail surface for app-emitted logs that this playbook uses while a suspected redirect-loop reproduction runs in a browser. The visible entries (`2026-06-07` timestamps, HTTP `POST` calls with `Content-Type: application/json`, `Response status 200` from OpenTelemetry exporter transmissions to `koreacentral-0.in.applicationinsights.azure.com/v2.1/track`) show the line-by-line format this blade prints — application log lines from the running worker appear in the same pane while it is open. The `Instances` dropdown is the per-instance tail selector visible above the streaming pane; the `Lookback period: Last 30 minutes` chip bounds the buffer to the reproduction window. The toolbar's `Log Level` filter is the noise control you adjust when the OpenTelemetry exporter chatter visible in this baseline (`Items received 3, Items accepted 3`) drowns out the lines you actually need to read.
...
| Command | Purpose |
|---------|---------|
| `az webapp show --resource-group <resource-group> --name <app-name>` | Shows the web app resource so you can inspect the current site configuration and host bindings before deeper auth checks. |
...
| `az webapp show --resource-group <resource-group> --name <app-name> --query "{defaultHostName:defaultHostName,httpsOnly:httpsOnly,hostNames:hostNames}" --output json` | Shows the hostname and HTTPS settings that must align with the redirect and callback URLs in use. |
| `--resource-group <resource-group>` | Selects the resource group that contains the target web app or related resource. |
| `--name <app-name>` | Specifies the target web app name for this command. |
| `--query "{defaultHostName:defaultHostName,httpsOnly:httpsOnly,hostNames:hostNames}"` | Projects the selected hostname and HTTPS fields into a smaller object before formatting the output. |
| `--output json` | Formats the command output as JSON for full-fidelity inspection. |
