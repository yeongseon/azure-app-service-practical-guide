...
content_validation:
  ...
  core_claims:
    - claim: "ASP.NET apps must be deployed to Azure App Service as compiled binaries."
      source: "https://learn.microsoft.com/en-us/azure/app-service/configure-language-dotnet-framework"
      verified: true
...
| Command | Purpose |
|---------|---------|
| `az webapp show --resource-group <resource-group> --name <app-name> --query "{state:state,enabled:enabled,defaultHostName:defaultHostName}" --output table` | Shows the app state, enabled flag, and default hostname so you can confirm the Windows site object is up during IIS startup triage. |
