...
The `Logs` blade is where the cross-signal queries below are pasted - this capture shows the Application Insights `Logs` experience (`ai-test-20251107`), but the workspace-based Log Analytics blade has the same `New Query 1` tab and `Run` toolbar. Correlation queries in this pack combine `AppServiceHTTPLogs` and `AppServicePlatformLogs` via `union` or shared time bins, so they emit multi-series timecharts that render inline in the lower pane after `Run`. Keep `Time range: Last 24 hours` as the starting window so latency, error, and restart signals can be compared across the full ingestion-delayed range; tighten the inline `Time range` selector once an incident window is identified. The empty `Query history` pane and the placeholder `Type your query here or click one of the queries to start` confirm a fresh session.
