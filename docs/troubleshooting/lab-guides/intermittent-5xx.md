...
#### 4.6.1 Log Analytics KQL query for status distribution by endpoint (portal verification)

To cross-verify the aggregate 200 / 499 status mix summarized in §4.6 (60 rows from `kql-http-20260404T060610Z.json`, split 29 `200` / 31 `499`, with `/slow` accounting for 40 rows and `/fast` for 12) is a real property of the raw `AppServiceHTTPLogs` rows and not a JSON-parse artifact of the sanitized export, the Log Analytics workspace Logs blade was opened against the same workspace from a fresh live reproduction and a per-endpoint status distribution query was executed interactively against the current live data:

![Azure portal Log Analytics workspace Logs blade for workspace law-lab5xx-o4evin7tt76tc with breadcrumb "Home > law-lab5xx-o4evin7tt76tc". Blade heading reads "law-lab5xx-o4evin7tt76tc | Logs" and the sub-heading reads "Log Analytics workspace". The left navigation shows Logs highlighted, with sibling entries Overview, Activity log, Access control (IAM), Tags, Diagnose and solve problems, Resource visualizer, Settings, Classic, Monitoring, Automation, and Help. The query editor tab reads "New Query 1*" and the KQL editor shows a four-line query: line 1 `AppServiceHTTPLogs`, line 2 `| where TimeGenerated > ago(1h)`, line 3 `| summarize total=count(), s200=countif(ScStatus==200), s499=countif(ScStatus==499) by CsUriStem`, line 4 `| order by total desc`. The query bar has a green Run button, Time range set to "Set in query", Show set to "1000 results", and the mode dropdown is on "KQL mode". The Results tab is active (sibling tab: Chart). The grid has four columns "CsUriStem", "total", "s200", "s499" with 5 rows: `/slow` 20 7 13, `/fast` 3 2 1, `/diag/stats` 2 1 1, `/` 1 1 0, `/health` 1 1 0. Bottom left shows query duration "0s 515ms" and "Display time (UTC+00:00)". Bottom right shows "Query details" and pagination "1 - 5 of 5".](../../assets/troubleshooting/log-analytics/09-5xx-status-by-endpoint-kql.png)

**Purpose**: Provide an independent Portal-side execution of a per-endpoint status distribution query so a reviewer can confirm that the mixed 200 / 499 signature in §4.6 (roughly 60% 499 on `/slow`, 58% 499 on `/fast`) is a real property of the raw `AppServiceHTTPLogs` rows the Log Analytics service holds — and can also see that the failure mode reproduces with the same shape (majority 499 on `/slow`, minority 499 bleeding into `/fast`, zero 5xx anywhere) when the trigger is re-run against a freshly deployed lab instance.

**Look for**:

- Blade heading reads "law-lab5xx-o4evin7tt76tc | Logs" and the sub-heading reads "Log Analytics workspace" — this confirms the query ran against the same workspace attached to this lab's Web App via diagnostic settings, matching the workspace referenced by the sanitized JSON exports in §4.1.
- The KQL editor shows exactly the query `AppServiceHTTPLogs | where TimeGenerated > ago(1h) | summarize total=count(), s200=countif(ScStatus==200), s499=countif(ScStatus==499) by CsUriStem | order by total desc` — `countif(ScStatus==200)` and `countif(ScStatus==499)` are conditional counts that partition each row group by status code without requiring separate pivot queries. Rows with any other status (e.g., 500, 502, 503) would not be counted in either the `s200` or `s499` columns and would show as `total > s200 + s499`.
- The Results grid shows exactly 5 rows (pagination "1 - 5 of 5") and every row satisfies `total == s200 + s499` — no third status class exists in this window. This falsifies the hypothesis "the app is returning genuine 5xx errors": every failure recorded is a `499` (client aborted before the server sent a response), not a `500` / `502` / `503` (server-side error).
- The `/slow` row shows `total 20, s200 7, s499 13` — 65% of `/slow` requests were aborted by the client at the 45-second `curl --max-time` boundary because the Gunicorn sync worker was still executing the artificial 5-15 s `time.sleep()` inside the handler when the client gave up.
- The `/fast` row shows `total 3, s200 2, s499 1` — a supposedly sub-100 ms endpoint recorded a 499 in the same window. This is the "starvation bleed" signature: `/fast` inherited the client-timeout fate because both sync workers were occupied serving `/slow` when the `/fast` request arrived, so it sat in the socket backlog until the client aborted.
- The `/diag/stats` row shows `total 2, s200 1, s499 1` — a low-volume secondary starvation-bleed signal from the diagnostic endpoint itself getting queued behind occupied workers, confirming that even in-app diagnostic endpoints suffer when the sync worker pool is saturated.
- The `/` and `/health` rows each show `total 1, s200 1, s499 0` — probe cadence during the trigger window; these endpoints exist for verification (see §5) and should not be interpreted as failure-mode signals.
- Query duration in bottom left reads a small millisecond value (here `0s 515ms`) — confirming the workspace is not throttled and the query hit indexed data, so the returned row count and status distribution are authoritative and not a partial-scan timeout.

**Expected result**: The 5 rows returned by the Portal show `/slow`, `/fast`, and `/diag/stats` with a mix of 200 and 499 statuses (with `/slow` and `/fast` as the dominant starvation signal and `/diag/stats` as low-volume bleed from the in-app diagnostic endpoint) and `/` and `/health` returning only 200, matching §4's overall verdict that the failure mode is worker starvation causing client aborts, not server errors: the app is healthy at the HTTP layer but sync workers cannot service concurrent long-running requests fast enough, so clients time out and record 499. If instead the Portal query returned rows with `ScStatus` 500, 502, or 503 in a non-zero third-column count (added as an additional `countif(ScStatus>=500 and ScStatus<600)` in a variant of this query), the verdict would need to be re-graded to "confirmed with outage" because the failure mode would have progressed from client-side timeouts to server-side worker recycles or platform-level errors.

**Next step**: If a future reproduction shows the `/slow` `s499` count at or near 0 (i.e., almost all `/slow` requests succeeded within the 45-second client timeout), the trigger is not actually saturating the workers — inspect `labs/intermittent-5xx/trigger.sh` for a concurrency regression (the default is 20 concurrent `/slow` requests against 2 sync workers, producing 10 requests-worth of demand per worker). Conversely, if `/fast` shows `s499 == total` (every `/fast` request aborted), the sleep distribution in `labs/intermittent-5xx/app/app.py` may have been widened beyond the intended 5-15 s range, holding workers so long that even burst-mode `/fast` requests cannot be serviced within their timeout.
...
#### 4.7.1 Log Analytics KQL query for latency percentiles by endpoint (portal verification)

To cross-verify the per-endpoint latency profile summarized in §4.7 (`/slow` avg 36,440 ms / p95 44,894 ms / max 44,921 ms across 40 rows; `/fast` avg 11,047 ms / p95 14,484 ms / max 44,865 ms across 12 rows) is a faithful reduction of the raw `TimeTaken` column and not a JSON-parse rounding artifact, the same Log Analytics workspace Logs blade was reopened and a per-endpoint per-status percentile query was executed interactively against the current live data. Splitting `by CsUriStem, ScStatus` in one query is important because the aggregate row-level table in §4.7 combines both status classes and hides the client-timeout signature inside the `/slow` average:

![Azure portal Log Analytics workspace Logs blade for workspace law-lab5xx-o4evin7tt76tc with breadcrumb "Home > law-lab5xx-o4evin7tt76tc". Blade heading reads "law-lab5xx-o4evin7tt76tc | Logs" and the sub-heading reads "Log Analytics workspace". The left navigation shows Logs highlighted, with sibling entries Overview, Activity log, Access control (IAM), Tags, Diagnose and solve problems, Resource visualizer, Settings, Classic, Monitoring, Automation, and Help. The query editor tab reads "New Query 1*" and the KQL editor shows a five-line query: line 1 `AppServiceHTTPLogs`, line 2 `| where TimeGenerated > ago(1h)`, line 3 `| where CsUriStem in ("/slow", "/fast")`, line 4 `| summarize avgMs=avg(TimeTaken), p95Ms=percentile(TimeTaken, 95), maxMs=max(TimeTaken) by CsUriStem, ScStatus`, line 5 `| order by p95Ms desc`. The query bar has a green Run button, Time range set to "Set in query", Show set to "1000 results", and the mode dropdown is on "KQL mode". The Results tab is active (sibling tab: Chart). The grid has five columns "CsUriStem", "ScStatus", "avgMs", "p95Ms", "maxMs" with 4 rows sorted by p95Ms desc: `/slow` 499 44652.38461538462 44693 44693, `/fast` 499 44548 44548 44548, `/slow` 200 20850.14285714286 33975 33975, `/fast` 200 4997.5 9992 9992. Bottom left shows query duration "1s 37ms" and "Display time (UTC+00:00)". Bottom right shows "Query details" and pagination "1 - 4 of 4".](../../assets/troubleshooting/log-analytics/09-5xx-latency-percentiles-kql.png)

**Purpose**: Provide an independent Portal-side execution of a per-endpoint per-status percentile query so a reviewer can confirm three claims from §4.7: (1) `/slow` requests that ended in a `499` cluster tightly at the 45-second client timeout because that is when the client gave up, (2) `/slow` requests that returned `200` fall into the 5-15 s app-simulated sleep range plus queueing delay, and (3) `/fast` requests that returned `200` are inflated well above the sub-100 ms baseline because they share the same sync worker pool as `/slow`.

**Look for**:

- The `/slow` `499` row shows `avgMs ≈ 44652, p95Ms 44693, maxMs 44693` — all three statistics cluster within ~41 ms of each other and sit ~307-348 ms below the 45-second `curl --max-time 45` client timeout, confirming that the 499 is the client giving up shortly before the configured deadline (the ~350 ms gap absorbs TCP FIN round-trip and the client's event-loop scheduling latency) and not a variable-latency degradation. If `avgMs` were, say, 30,000 ms with `maxMs` at 90,000 ms instead, that would suggest an intermediate load balancer timeout at a different value.
- The `/fast` `499` row shows `avgMs 44548` — this is the single `/fast` request that was aborted, and its latency being nearly identical to the `/slow` `499` cluster proves the `/fast` request was queued behind a `/slow` request and inherited the same 45-second client deadline, not that `/fast` itself takes 45 seconds inside the handler.
- The `/slow` `200` row shows `avgMs ≈ 20850, p95Ms 33975, maxMs 33975` — successful `/slow` requests average ~20.9 s (roughly a ~15 s queue wait plus the app's 5-15 s intended sleep), and the slowest that still returned 200 (33,975 ms) sat in the queue for ~20 seconds before its sync worker was free to run the sleep and return. The Portal aggregate view does not expose the minimum, so any statement about the fastest `/slow` 200 in this window requires querying raw rows directly — the "Representative rows" table in §4.8 shows only a single `/slow` 200 sample (37,019 ms) drawn from a different sanitized reproduction and is not sufficient to establish an observed minimum for the fresh window shown here.
- The `/fast` `200` row shows `avgMs 4997.5, p95Ms 9992, maxMs 9992` — a supposedly sub-100 ms endpoint took on average 5 seconds and up to 10 seconds at p95 to return successfully. This is a 50× to 100× latency inflation and is the single strongest piece of evidence that `/fast` shares the same worker pool as `/slow` (there is no `--worker-class=gthread` or async worker in the Gunicorn config; both endpoints compete for the same 2 sync worker slots).
- Rows are sorted by `p95Ms desc` — the top two rows are both `499` (44,693 ms and 44,548 ms), then the next two are `200` (33,975 ms and 9,992 ms). The fact that no `200` row exceeds any `499` row's `p95Ms` confirms the client-timeout boundary (~45 s) is strictly above the app-side latency ceiling, so no successful request "runs longer than the timeout allows" and every request over the deadline is aborted client-side.
- Query duration in bottom left reads `1s 37ms` — slightly longer than the §4.6.1 status-distribution query (`0s 515ms`) because `percentile()` is more expensive than `countif()`, but still well under the 30-second workspace query timeout, so the returned percentile values are authoritative and not a partial-scan estimate.

**Expected result**: The two `499` rows cluster at the ~45 s client-timeout boundary, the `/slow` `200` row falls in the 5-15 s app sleep range plus queueing, and the `/fast` `200` row shows 50-100× latency inflation over its sub-100 ms baseline — collectively confirming that the failure signature is client-side timeouts driven by sync-worker saturation, not by app-side crashes or platform-side outages. If instead the Portal query returned a `/fast` `200` row with `p95Ms` under 100 ms, the trigger did not actually saturate the workers (perhaps because the workers were recycled between the trigger and the query, or the trigger arrived before the worker pool was fully warmed) — in that case this cross-verification is invalid and the trigger must be re-run before drawing conclusions from the raw KQL export in §4.8.

**Next step**: If a future reproduction shows `maxMs` on the `/slow` `499` row substantially higher than the `curl --max-time` boundary (for example 60+ seconds instead of ~45 seconds), the client-side timeout was likely raised or removed — re-check `labs/intermittent-5xx/trigger.sh` for a `--max-time` regression. If instead `maxMs` on the `/slow` `200` row approaches the 45-second `curl --max-time` boundary itself (for example 40+ seconds when the current baseline is ~34 seconds), the sleep distribution in `labs/intermittent-5xx/app/app.py` was likely widened beyond the intended 5-15 s range, making the sync-worker saturation more aggressive than this lab's hypothesis models and shifting the failure mode toward "confirmed with outage" rather than "supported (starvation causes client aborts)".
