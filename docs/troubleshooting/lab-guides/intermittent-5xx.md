...
content_validation:
  ...
  core_claims:
    ...
    - claim: "App Service console logs are used in this lab to inspect runtime warnings and timeout signals."
      source: "https://learn.microsoft.com/azure/app-service/troubleshoot-diagnostic-logs"
      verified: true
    - claim: "App Service platform logs are used in this lab to inspect platform lifecycle events during failures."
      source: "https://learn.microsoft.com/azure/app-service/troubleshoot-diagnostic-logs"
      verified: true
---
...
The `Logs` blade is where you paste the KQL snippets below. Open the `New Query 1` tab on the Log Analytics workspace attached to this app, set `Time range: Last 24 hours` to cover the intermittent failure window (the lab's 5xx pattern usually spans hours, not minutes), and keep `Show: 1000 results` so summarized status distributions are not truncated. The `Queries hub` button in the top-right gives you a saved-query library; once you have run one of the snippets below, save it there so the next on-call engineer can re-run the same triage in one click. The empty `Query history` pane indicates this is a fresh session - paste the first KQL block to populate it.
