...
![Azure portal Overview blade for the asp-test-20251107 App Service Plan (Linux plan). The header shows the plan name with Pin, Favorite, More content actions, and Close buttons; below it the toolbar exposes Delete and Send us your feedback. The Essentials panel on the left lists Resource group rg-test-20251107, Status Ready, Location Korea Central, Subscription Visual Studio Enterprise Subscription, Subscription ID 00000000-0000-0000-0000-000000000000, and Tags (Add tags). The right side of Essentials lists Pricing plan P0v3, Instance count 1, App(s) / Slots 1 / 1, Operating System Linux, and Zone redundant Disabled. Four pinned metric charts are visible below Essentials: CPU Percentage (Avg 4.45% with peaks near 10%), Memory Percentage (Avg 46.3333% as a near-flat band around 45%), Data In (Sum 15.85 kB with intermittent spikes up to 4 kB), and Data Out (Sum 5.46 kB shown at the bottom-left of the visible area). The left navigation panel highlights Overview as the active blade, with Activity log, Access control (IAM), Tags, Diagnose and solve problems, Events (preview), Resource visualizer, Settings (collapsed), Monitoring (collapsed), Automation (collapsed), and Help (collapsed) listed below.](../../assets/troubleshooting/app-service-plan/01-asp-overview.png)

The `App Service Plan` Overview blade is the canonical place to confirm the storage tier that drives the 10 GB `/home` quota cited above - the `Pricing plan` value on the right side of `Essentials` (here `P0v3`) maps directly to the persistent-storage allotment in the official App Service limits table, so changing it (Scale up) is the platform-level lever for raising or lowering the quota this lab exhausts. The `App(s) / Slots: 1 / 1` field is the second critical signal because `/home` storage is shared across every app and deployment slot bound to the same plan: a second app on the same plan would consume from the same 10 GB pool described in section 1.3's persistent-vs-ephemeral semantics, which is the most common real-world source of "I didn't change anything and now I'm out of space". The `Operating System: Linux` and `Zone redundant: Disabled` fields anchor the rest of the lab's assumptions - the Linux storage model in section 1.2 only applies when this field is `Linux`, and zone-redundant plans have different recovery characteristics that would change the recovery actions in section 3.12. Use this blade as your first stop when investigating "no space left on device" errors in production: confirm the SKU, then click the `Pricing plan` link to reach the Scale up blade if the long-term fix is a tier change, or click into the plan's `Apps` blade to enumerate every app competing for the shared `/home` quota.
