...
![Azure portal Overview blade for the asp-test-20251107 App Service Plan (Linux plan). The header shows the plan name with Pin, Favorite, More content actions, and Close buttons; below it the toolbar exposes Delete and Send us your feedback. The Essentials panel includes View Cost and JSON View links at its top-right; the left side lists Resource group rg-test-20251107, Status Ready, Location Korea Central, Subscription Visual Studio Enterprise Subscription, Subscription ID 00000000-0000-0000-0000-000000000000, and Tags (Add tags). The right side of Essentials lists Pricing plan P0v3, Instance count 1, App(s) / Slots 1 / 1, Operating System Linux, and Zone redundant Disabled. Four pinned metric charts are visible below Essentials: CPU Percentage (Avg 4.45% with peaks near 10%), Memory Percentage (Avg 46.3333% as a near-flat band around 45%), Data In (Sum 15.85 kB with intermittent spikes up to 4 kB), and Data Out (only the upper chart area is visible; the numeric summary below the chart is cut off below the fold). The left navigation panel highlights Overview as the active blade, with Activity log, Access control (IAM), Tags, Diagnose and solve problems, Events (preview), Resource visualizer, Settings (collapsed), Monitoring (collapsed), Automation (collapsed), and Help (collapsed) listed below.](../../assets/troubleshooting/app-service-plan/01-asp-overview.png)

The `App Service Plan` Overview blade is the canonical place to confirm the platform-side configuration that bounds this lab - the `Pricing plan` value (here `P0v3`) controls compute resources (CPU, memory, scale-up options), while the 10 GB `/home` quota cited above is set by the App Service Linux storage model rather than the SKU itself, so this blade is more useful for confirming the OS context and shared-storage scope than for raising the disk quota directly. The `App(s) / Slots: 1 / 1` field is a critical signal because `/home` storage is shared across every app and deployment slot bound to the same plan: a second app on the same plan would consume from the same `/home` pool described in section 1.3's persistent-vs-ephemeral semantics, which is the most common real-world source of "I didn't change anything and now I'm out of space". The `Operating System: Linux` field anchors the rest of the lab's assumptions because the Linux storage model in section 1.2 only applies when this field reads `Linux` - on Windows plans the storage layout and quota scaling differ. Use this blade as your first stop when investigating "no space left on device" errors in production: confirm the SKU and OS first, click the `Pricing plan` link to reach the Scale up blade if compute starvation is in scope, or expand the left navigation to locate the plan's `Apps` blade and enumerate other apps that may be competing for the shared `/home` quota.
