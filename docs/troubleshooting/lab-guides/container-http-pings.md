...
The `Log stream` blade is the fastest way to see Front Door's HTTP probes hitting your container in real time, before the KQL queries below have ingested into Log Analytics. Switch to the `Platform` radio to confirm the platform's `Pinging warmup path` and `Container did not start within expected time limit` messages, then back to `Runtime` to verify whether Gunicorn ever printed `Listening at: http://0.0.0.0:<port>` - the absence of that line on the expected `WEBSITES_PORT` is the smoking gun for this failure mode. Pin the `Instances` dropdown to the single worker hash so the probe cadence is uninterrupted. Use the KQL block below to quantify probe windows once you have visually confirmed the live behavior here.
