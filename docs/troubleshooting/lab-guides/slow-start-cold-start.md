...
The `Log stream` blade gives you a live view of startup progress without ingestion delay, which is critical when measuring cold-start phases that are over in seconds. Toggle the `Platform` radio (instead of `Runtime`) to catch `CreatingContainer`, `PullingImage`, and `WarmUpProbeSucceeded` lifecycle events as they fire; switch back to `Runtime` to see the Gunicorn `Booting worker` and `Listening at` lines that mark application-startup completion. Pin the `Instances` dropdown to a single worker hash so you can measure one instance's cold-start timeline cleanly. After capturing the live trace here, run the KQL queries below to aggregate the same signals across multiple restarts for trending.
