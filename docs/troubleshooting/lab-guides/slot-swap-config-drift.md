...
The `Activity log` blade is the Portal anchor for swap-drift investigations - it gives you the audit trail of `Swap Web App Slots` operations with timestamps, initiator, and operation status, while the KQL queries below give you the application-side evidence. Apply `Timespan: Last 6 hours` and `Resource: app-test-20251107` filters to scope down to the swap window, then look for the `Swap Web App Slots` operation row with `Succeeded` status - this confirms the platform-level swap completed even when users report config "drift". The `Event initiated by` column shows which principal triggered the swap, which is useful when correlating against the sticky-vs-non-sticky settings table in section 2. Use the KQL block below to then verify the application-side restart and request behavior aligns with the audited swap event.
