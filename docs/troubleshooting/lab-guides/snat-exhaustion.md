...
![Azure portal Log Analytics workspace Logs blade for workspace log-labsnat-6kr7mkxulwioo with breadcrumb "Home > log-labsnat-6kr7mkxulwioo". Blade heading reads "log-labsnat-6kr7mkxulwioo | Logs" and the sub-heading reads "Log Analytics workspace". The left navigation shows Logs highlighted, with sibling entries Overview, Activity log, Access control (IAM), Tags, Diagnose and solve problems, Resource visualizer, Settings, Classic, Monitoring, Automation, and Help. The query editor tab reads "New Query 1*" and the KQL editor shows a five-line query: line 1 `AppServiceHTTPLogs`, line 2 `| where TimeGenerated > ago(2h)`, line 3 `| where CsUriStem == '/outbound'`, line 4 `| summarize Count = count() by ScStatus`, line 5 `| order by ScStatus asc`. The query bar has a green Run button, Time range set to "Set in query", Show set to "1000 results", and the mode dropdown is on "KQL mode". The Results tab is active (sibling tab: Chart). The grid has two columns "ScStatus", "Count" with 3 rows: `200` 12, `499` 152, `502` 4. Bottom left shows query duration "1s 531ms" and "Display time (UTC+00:00)". Bottom right shows "Query details" and pagination "1 - 3 of 3".](../../assets/troubleshooting/log-analytics/10-snat-outbound-status-distribution-kql.png)
...
![Azure portal Log Analytics workspace Logs blade for workspace log-labsnat-6kr7mkxulwioo with breadcrumb "Home > log-labsnat-6kr7mkxulwioo". Blade heading reads "log-labsnat-6kr7mkxulwioo | Logs" and the sub-heading reads "Log Analytics workspace". The left navigation shows Logs highlighted, with sibling entries Overview, Activity log, Access control (IAM), Tags, Diagnose and solve problems, Resource visualizer, Settings, Classic, Monitoring, Automation, and Help. The query editor tab reads "New Query 1*" and the KQL editor shows a six-line query: line 1 `AppServiceHTTPLogs`, line 2 `| where TimeGenerated > ago(2h)`, line 3 `| where CsUriStem == '/outbound'`, line 4 `| project TimeGenerated, CsUriStem, ScStatus, TimeTaken`, line 5 `| order by TimeTaken desc`, line 6 `| take 10`. The query bar has a green Run button, Time range set to "Set in query", Show set to "1000 results", and the mode dropdown is on "KQL mode". The Results tab is active (sibling tab: Chart). The grid has four columns "TimeGenerated [UTC]", "CsUriStem", "ScStatus", "TimeTaken" with 10 rows all showing CsUriStem `/outbound` and ScStatus `499`. TimeTaken values from top to bottom (in ms): 240036, 240028, 240027, 240027, 240021, 240020, 240019, 240018, 240017, 240015. The 10 TimeGenerated values span 7/3/2026 6:04:20.115 AM to 7/3/2026 6:27:02.767 AM UTC (a ~22 minute 42 second wall-clock window). Bottom left shows query duration "0s 912ms" and "Display time (UTC+00:00)". Bottom right shows "Query details" and pagination "1 - 10 of 10".](../../assets/troubleshooting/log-analytics/10-snat-outbound-tail-latency-kql.png)
