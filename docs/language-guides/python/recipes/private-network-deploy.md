...
#### Portal view: Access Restrictions blade (public network access control)
...
The Access Restrictions blade is the Portal counterpart to the `az webapp config access-restriction` commands this recipe layers around private deployment. The visible `Public network access` selector — `Enabled from all networks` / `Enabled from select virtual networks and IP addresses` / `Disabled` — is the same three-state property the recipe flips to `Disabled` once the private endpoint is verified, with the on-blade notice describing the exact effect: blocking all incoming traffic except from private endpoints. The `Site access and rules` section with its `Main site` and `Advanced tool site` tabs is the surface where the recipe's allow-list rules for build pipelines or internal jump hosts appear in the `Priority`/`Name`/`Source`/`Action`/`HTTP headers` table. Use this blade after running each recipe access-restriction step to confirm the rule, action, and unmatched-rule policy match the intended private-deploy posture.
