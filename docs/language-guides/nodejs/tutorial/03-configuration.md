...
The `Configuration > General settings` blade is the Portal verification surface where the runtime controls referenced across this Node.js tutorial appear. In the visible Platform settings list, `HTTPS only`, `Always on`, `WebJobs runtime`, `Session affinity`, and `Minimum Inbound TLS Version` are the same toggles you would otherwise configure via `az webapp config set`. This screenshot also makes the default state concrete: `Always on` and `HTTPS only` are both unchecked here, so you should not assume production-ready defaults after `az webapp up` first creates the Express app. Use this blade alongside the `az webapp config appsettings set` and `az webapp config connection-string set` flows shown above to confirm both runtime controls and configuration values applied to the Node.js app.
