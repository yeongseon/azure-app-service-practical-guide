...
The Environment variables blade with the `App settings` tab selected is the Portal surface where Key Vault references created by this recipe live. The visible rows here — `APPLICATIONINSIGHTS_CONNECTION_STRING`, `APPLICATIONINSIGHTSAGENT_EXTENSION_ENABLED`, `ApplicationInsightsAgent_EXTENSION_VERSION`, `SCM_DO_BUILD_DURING_DEPLOYMENT`, and `WEBSITE_HTTPLOGGING_RETENTION_DAYS` — share the same `Name`, `Value`, `Deployment slot setting`, and `Source` layout that any `@Microsoft.KeyVault(...)` app setting uses when added with `az webapp config appsettings set`. The `Pull reference values`, `Refresh`, `Show values`, and `Advanced edit` actions in the toolbar are the Portal-side controls available on this blade. Use this view to inspect Key Vault-backed settings alongside the platform-managed entries for the Spring Boot app.
