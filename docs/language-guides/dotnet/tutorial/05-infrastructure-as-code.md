...
After running `az deployment group create --template-file ./infra/main.bicep`, the resource group overview is the Portal surface for checking what the deployment produced. The visible `Resources` tab shows `App Service`, `App Service Plan`, `Application Insights`, and other related rows grouped under one resource group, while the `Deployments` entry in Essentials is the link to follow when you need deployment history after a partial failure. The `Export template` button is also useful when you want to compare the deployed ARM state with the Bicep source in your repository.
