...
After running `az deployment group create --template-file ./infra/main.bicep`, the resource group overview is the Portal surface for checking what the .NET Bicep deployment produced. The visible `Resources` tab shows how Azure inventories related resources as separate rows in one resource group, including `App Service`, `App Service Plan`, and `Application Insights` — the three resources this tutorial's Bicep template defines for the ASP.NET Core 8 app. The `Deployments` entry in Essentials is the link to follow when you need to inspect deployment history after a partial failure. The `Export template` button is also useful when you want to compare the deployed ARM state with the Bicep source in your repository.
