...
Most CLI verbs in this cheatsheet have a direct visual counterpart on this blade. The command bar `Stop`, `Restart`, and `Delete` buttons map to `az webapp stop`, `az webapp restart`, and `az webapp delete`; `Browse` opens the `Default domain` URL surfaced in Essentials and `Download publish profile` mirrors `az webapp deployment list-publishing-profiles`. The Essentials panel (`Resource group`, `App Service Plan`, `Operating System`, `Default domain`) shows the same fields returned by `az webapp show`; the Hosting block (`SKU and size: Premium0V3 (P0v3)`, `Instance Count: 1`) mirrors `az appservice plan create --sku` and `az monitor autoscale` settings; and the visible `Scale up` link is the GUI equivalent of `az appservice plan update --sku`. The collapsed `Settings` and `Deployment` groups in the left nav hold the blades behind `az webapp config appsettings`, `az webapp config connection-string`, and `az webapp deploy`. Use this view to verify state after running CLI commands - `Last deployment` and `Runtime status: Healthy` are the same fields you would otherwise query via `az webapp log deployment list` and the JSON output of `az webapp show`.
