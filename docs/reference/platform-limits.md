...
The `Scale up (App Service plan)` blade is where you change the SKU that drives most of the limits in the Scale and Plan Limits table above. The visible `Production V3` card lists `P0v3`, `P1v3`, `P2v3`, `P3v3`, and the memory-optimized `P1mv3` through `P5mv3`, which is the Premium tier family summarized in the table as `Premium (P1V3 and above)`. Because this blade is scoped to the App Service plan and exposes an `Upgrade` action, it is the right place to confirm whether plan tier is part of a capacity or saturation problem. Use this view to verify the current plan tier before concluding that a symptom is application-only.
