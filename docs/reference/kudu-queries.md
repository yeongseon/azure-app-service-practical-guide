...
The Kudu (SCM) endpoints listed above all live at `https://${APP_NAME}.scm.azurewebsites.net` - the visible `Default domain: app-test-20251107.azurewebsites.net` in Essentials is the matching app domain, and the SCM hostname for this app would be `app-test-20251107.scm.azurewebsites.net`. The Portal entry point to Kudu is the collapsed `Development Tools` group visible in the left nav: expanding it surfaces `Advanced Tools (Kudu)`, which opens the SCM site in a new tab using the signed-in Entra session instead of the `--user`/`--password` basic auth used by the `curl` snippets above. The Essentials `App Service Plan: asp-test-20251107 (P0v3: 1)` and `Operating System: Linux` confirm a Linux Code app on Premium V3, so the Linux custom container caveat does not apply here. The `Last deployment: Successful on Thursday, November 6, 10:58:01 PM` value under Deployment Center is the same record that `GET /api/deployments/latest` returns, and `Runtime Stack: Python - 3.11` is what `GET /api/environment` exposes through the `WEBSITE_STACK` and `LANG_VERSION` variables.
